...
public:
  ...
  bool is_inverted() const {
    return (reinterpret_cast<size_t>(m_ref) & static_cast<size_t>(1)) ==
           static_cast<size_t>(1);
  }
  void invert() {
    m_ref = reinterpret_cast<aig *>(reinterpret_cast<size_t>(m_ref) ^
                                    static_cast<size_t>(1));
  }
  aig *ptr() const {
    return reinterpret_cast<aig *>(reinterpret_cast<size_t>(m_ref) &
                                   ~static_cast<size_t>(1));
  }
  aig *ptr_non_inverted() const {
    SASSERT(!is_inverted());
    return m_ref;
  }
  bool is_null() const { return m_ref == nullptr; }
  friend bool operator==(aig_lit const &r1, aig_lit const &r2) {
    return r1.m_ref == r2.m_ref;
  }
  friend bool operator!=(aig_lit const &r1, aig_lit const &r2) {
    return r1.m_ref != r2.m_ref;
  }
  aig_lit &operator=(aig_lit const &r) {
    m_ref = r.m_ref;
    return *this;
  }
  static aig_lit null;
...
#if Z3DEBUG
inline bool is_true(aig_lit const &r) {
  return !r.is_inverted() && r.ptr_non_inverted()->m_id == 0;
}
#endif
// inline bool is_false(aig_lit const & r) { return r.is_inverted() &&
// r.ptr()->m_id == 0; }
...
inline unsigned to_idx(aig *p) {
  SASSERT(!is_var(p));
  return p->m_id - FIRST_NODE_ID;
}
...
static void unmark_lit(ptr_vector<const aig_lit> &queue) {
  for (unsigned int i = 0; i < queue.size(); i++) {
    const aig_lit *lit = queue[i];
    lit->ptr()->m_mark = false;
  }
}
...
struct aig_eq {
  bool operator()(aig *n1, aig *n2) const {
    SASSERT(!is_var(n1));
    SASSERT(!is_var(n2));
    return n1->m_children[0] == n2->m_children[0] &&
...
struct aig_lit_lt {
  bool operator()(aig_lit const &l1, aig_lit const &l2) const {
    if (id(l1) < id(l2))
      return true;
    if (id(l1) == id(l2))
      return l1.is_inverted() && !l2.is_inverted();
    return false;
  }
};
...
struct aig_manager::imp {
  ...
  typedef u_map<unsigned> aigmap_t;
  aigmap_t m_aig_map;
  aigmap_t m_rev_aig_map;
  ...
  void delete_node(aig *n) {
    TRACE("aig_lit_count", tout << "deleting: "; display_ref(tout, n);
          tout << "\n";);
  ...
  */
  ...
  start:
    ...
    if (!is_var(n1)) {
      ...
      if (is_not_eq(a, r) || is_not_eq(b, r)) {
        if (sign1) {
          // substitution
          return r; // (not (a and b)) and r  --> r   IF  a = (not r) or b =
                    // (not r)
        } else {
          // contradiction
          return m_false; // (a and b) and r  --> false  IF a = (not r) or b =
                          // (not r)
        }
      }
      if (a == r) {
      ...
      if (!is_var(n2)) {
        ...
        if (!sign1 && !sign2) {
          // contradiction
          if (is_not_eq(a, c) || is_not_eq(a, d) || is_not_eq(b, c) ||
              is_not_eq(b, d))
            return m_false; // (a and b) and (c and d)  --> false  IF  a =
                            // not(c) OR a = not(d) or b = not(c) or b = not(d)
          // idempotence
          if (a == c || b == c) {
            r = d; // (a and b) and (c and d) --> (a and b) and d   IF a == c or
                   // b == c
            goto start;
          }
          // idempotence
          if (b == c || b == d) {
            l = a; //  (a and b) and (c and d) --> a and (c and d)  IF b == c or
                   //  b == d
            goto start;
          }
          // idempotence
          if (a == d || b == d) {
            r = c;
            goto start; //  (a and b) and (c and d) --> (a and b) and c   IF a
                        //  == d or b == d
          }
          // idempotence
          if (a == c || a == d) {
            l = b; //  (a and b) and (c and d) --> b and (c and d)  IF a == c or
                   //  a == d
            goto start;
          }
        }
        ...
        if (sign1 && sign2) {
          ...
          SASSERT(!(a == d &&
                    is_not_eq(b, c))); // cannot happen because we sort args
          // resolution
          if (is_not_eq(a, c) && b == d) {
            b.invert(); // (not (a and b)) and (not (a and (not b))) --> not b
            return b;
          }
          SASSERT(!(is_not_eq(a, d) &&
                    b == c)); // cannot happen because we sort args
        }
      }
    }
    ...
    if (!is_var(n2)) {
      ...
      if (is_not_eq(l, a) || is_not_eq(l, b)) {
        if (sign2) {
          // substitution
          return l; // l and (not (a and b)) --> l   IF  a = (not l) or b = (not
                    // l)
        } else {
          // contradiction
          return m_false; // l and (a and b) --> false  IF a = (not l) or b =
                          // (not l)
        }
      }
      if (a == l) {
  ...
  struct expr2aig {
    ...
    bool visit(expr *t) {
      if (is_app(t)) {
        ...
        if (tapp->get_family_id() == m.m().get_basic_family_id()) {
          ...
          case OP_TRUE:
            save_result(m.m_true);
            return true;
          case OP_FALSE:
            save_result(m.m_false);
            return true;
          case OP_EQ:
    ...
    void mk_ite(unsigned spos) {
      ...
      aig_lit r = m.mk_ite(m_result_stack[spos], m_result_stack[spos + 1],
                           m_result_stack[spos + 2]);
      save_node_result(spos, r);
    }
    ...
    void mk_iff(unsigned spos) {
      if (spos + 2 != m_result_stack.size())
        throw default_exception(
            "aig conversion assumes expressions have been simplified");
  ...
  */
  ...
  struct aig2expr {
    ...
    expr *get_cached(aig *n) {
      if (is_var(n)) {
        ...
        CTRACE("aig2expr", !is_cached(n), tout << "invalid get_cached for ";
               m.display_ref(tout, n); tout << "\n";);
        SASSERT(is_cached(n));
    ...
    void mk_and(aig *n) {
      ...
      expr *r = ast_mng.mk_not(
          ast_mng.mk_or(m_and_children.size(), m_and_children.data()));
      cache_result(n, r);
      TRACE("aig2expr", tout << "caching AND "; m.display_ref(tout, n);
            tout << "\n";);
    }
    ...
    void mk_ite(aig *n) {
      ...
      cache_result(n, r);
      TRACE("aig2expr", tout << "caching ITE/IFF "; m.display_ref(tout, n);
            tout << "\n";);
    }
    ...
    */
    expr *process_root(aig *r) {
      ...
      while (!m_frame_stack.empty()) {
        ...
        case AIG_AUX_AND:
          // do nothing
          TRACE("aig2expr", tout << "skipping aux AND "; m.display_ref(tout, n);
                tout << "\n";);
          break;
        case AIG_AND:
          mk_and(n);
    ...
    */
    ...
    void operator()(aig_lit const &l, expr_ref &r) { naive(l, r); }
  ...
  struct max_sharing_proc {
    ...
    bool improve_sharing_left(aig *o, aig_lit n) {
      ...
      aig_lit b = right(left(n));
      aig_lit c = right(n);
      TRACE("max_sharing", tout << "trying (and "; m.display_ref(tout, a);
            tout << " (and "; m.display_ref(tout, b); tout << " ";
            m.display_ref(tout, c); tout << "))\n";);
      ...
      TRACE("max_sharing", tout << "trying (and "; m.display_ref(tout, a);
            tout << " (and "; m.display_ref(tout, c); tout << " ";
            m.display_ref(tout, b); tout << "))\n";);
    ...
    bool improve_sharing_right(aig *o, aig_lit n) {
      ...
      TRACE("max_sharing", tout << "trying (and (and "; m.display_ref(tout, a);
            tout << " "; m.display_ref(tout, b); tout << ") ";
            m.display_ref(tout, c); tout << ")\n";);
      ...
      aig_lit ac = m.mk_and(a, c);
      TRACE("max_sharing", tout << "trying (and (and "; m.display_ref(tout, a);
            tout << " "; m.display_ref(tout, c); tout << ") ";
            m.display_ref(tout, b); tout << ")\n";);
      m.inc_ref(ac);
      if (ref_count(ac) > 1) {
    ...
    void improve_sharing_core(aig *o, aig_lit n) {
      if (!is_var(n)) {
        aig_lit l = left(n);
        if (!l.is_inverted() && ref_count(l) == 1 && !is_var(l) &&
            improve_sharing_left(o, n))
          return;
        aig_lit r = right(n);
        if (!r.is_inverted() && ref_count(r) == 1 && !is_var(r) &&
            improve_sharing_right(o, n))
          return;
      }
      save_result(o, n);
    }
    ...
    void process(aig *p) {
      ...
      start:
        frame &fr = m_frame_stack.back();
        aig *n = fr.m_node;
        TRACE("max_sharing", tout << "processing "; m.display_ref(tout, n);
              tout << " idx: " << fr.m_idx << "\n";);
        switch (fr.m_idx) {
        case 0:
          fr.m_idx++;
...
public:
  imp(ast_manager &m, unsigned long long max_memory, bool default_gate_encoding)
      : m_var_id_gen(0), m_node_id_gen(FIRST_NODE_ID), m_num_aigs(0),
        ...
        m_aig_map(aigmap_t()), m_rev_aig_map(aigmap_t()) {
  ...
  aig_lit mk_and(aig_lit r1, aig_lit r2) {
    ...
    TRACE("mk_and_bug", display(tout, r1); tout << "AND\n"; display(tout, r2);
          tout << "-->\n"; display(tout, r); tout << "\n";);
    return r;
  }
  ...
  template <typename S> aig_lit mk_aig(S const &s) {
    aig_lit r;
    r = m_true;
    inc_ref(r);
    try {
  ...
  unsigned int compute_idx(unsigned int i) const { return i / 2 - 1; }
  ...
  void update_aig_map(aig_lit const &r, bool is_output) {
    if (m_aig_map.contains(r.ptr()->m_id)) {
      return;
    }
    if (is_var(r.ptr())) {
    ...
    m_aig_map.insert(r.ptr()->m_id, m_max_used_id);
    m_rev_aig_map.insert(m_max_used_id, r.ptr()->m_id);
    m_max_used_id += 2;
  }

  void display_node(std::ostream &out, aig_lit const &r) const {
    if (r.is_inverted()) { // aig used odd number to represent negation
      out << (m_aig_map[r.ptr()->m_id] + 1);
    } else {
      out << m_aig_map[r.ptr()->m_id];
    }
  }
  ...
  void display_gate(std::ostream &out, aig_lit const &r, aig_lit const &r1,
    ...
    out << m_aig_map[r.ptr()->m_id]; // output must be even number
    out << " ";
    display_node(out, r1);
    out << " ";
    display_node(out, r2);
  ...
  void iterate(aig_lit const &r) {
    ...
    unmark_lit(queue);
    unmark_lit(gate_queue);
  }

  void display_aag(std::ostream &out, aig_lit const &r) {
    iterate(r);
    out << "aag " << m_input_num + m_gate_num << " " << m_input_num << " 0 "
  ...
  unsigned get_num_aigs() const { return m_num_aigs; }
};

aig_ref::aig_ref() : m_manager(nullptr), m_ref(nullptr) {}

aig_ref::aig_ref(aig_manager &m, aig_lit const &l)
    : m_manager(&m), m_ref(l.m_ref) {
  m.m_imp->inc_ref(l);
}
...
aig_manager::aig_manager(ast_manager &m, unsigned long long max,
                         bool default_gate_encoding) {
  m_imp = alloc(imp, m, max, default_gate_encoding);
}

aig_manager::~aig_manager() { dealloc(m_imp); }
...
aig_ref aig_manager::mk_ite(aig_ref const &r1, aig_ref const &r2,
                            aig_ref const &r3) {
...
void aig_manager::reset_aag_info() { m_imp->reset_aag_info(); }